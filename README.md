...
- **同一結果の抽選**: サーバー側で項目を等確率に抽選し、その項目内の停止位置もランダムに決め、停止角度と当選名を全員に同期します。境界のすぐ近くにも止まりますが、境界ちょうどの曖昧な位置は避けます。
- **シンプルなUI**: 追加した項目が円形セグメントに色分け表示され、自然な加速→減速のアニメーションで回転します。
- **項目管理**: 項目一覧を優先した高さ配分、40pxの削除ボタン、折りたたみ可能な履歴欄。狭い画面では縦スクロールで操作できます。
- **履歴復元**: 抽選時・全削除前・履歴復元前の項目セットを最新10件保存。同じセットが連続する場合はまとめます。
- **音と演出**: BGM・回転音・成功音・紙吹雪。右上の「音 ON / OFF」で端末ごとに切り替え、設定を保存します。OSの視差効果抑制設定では紙吹雪などの装飾演出を抑えますが、抽選状況を全員が確認できるようルーレット自体は回転します。
...
- サーバ状態（`RouletteState`）
  ...
  - `spinId / spinStartedAt / resultItem` 抽選の識別子・開始時刻・当選項目
  - `history` 保存した項目セット（最大10件）
- クライアント操作
  - 項目追加: `room.send("add_item", { item })`
  - 項目削除: `room.send("remove_item", { item })`
  - 回す: `room.send("spin")`
- サーバ挙動
  - `spin` 受信時、項目インデックスを等確率に抽選し、その項目内でも停止位置を一様に抽選して `targetRotation` と `resultItem` を設定し `isSpinning=true`
  - `targetRotation` は64ビット浮動小数点で送信し、境界付近の丸めでポインターが隣の項目へずれることを防止
  - 回転中は追加・削除・全削除・履歴復元・再抽選をサーバ側でも拒否
  - 5 秒後に `isSpinning=false` に戻す（クライアントのアニメ時間と一致）
- クライアント描画
  - サーバから受け取った `targetRotation` を基準に、現在角度から 3 回転＋相対回転で合計回転角を算出
  - 自然なイージングで 5 秒回転し停止。サーバの当選名を表示
  - 途中参加は開始時刻から経過時間を反映。抽選後の参加でも同じ角度・当選名を表示
...
- サーバ: `apps/server/.env`（必要に応じて作成）
  ...
  - `HISTORY_STORE=memory`（任意）: Firestoreを使わないローカル確認用。履歴はプロセス終了時に消えます。省略時は従来どおりFirestoreを使用します。
...
クライアントのTypeScriptは型チェックのみ行い、JavaScriptの生成はViteに任せます。`src` に生成済みの `.js` を混在させないことで、開発時に古いコードが読み込まれるのを防ぎます。

回帰テストでは複数クライアントの追加・削除同期、不正入力、回転中の全操作ロック、途中参加への結果共有、全削除・復元前の履歴保存、履歴10件制限を確認します。

ブラウザでの確認手順:

1. ローカルサーバーを `HISTORY_STORE=memory` で起動し、クライアントを2つのタブで開く（ローカルのモックは同じチャンネルに参加します）。
2. 12項目以上と複数の履歴セットを用意し、1920×1080・1536×864・1280×720・960×540・390×844で一覧と削除ボタンを確認。高さが足りない場合はページを縦スクロールできます。
3. 一覧の最後までスクロールして削除し、履歴の開閉・全削除後の復元を確認。
4. 一方から回し、両方で編集不可・同じ結果になることを確認。回転中に別のタブで参加しても同じ結果になることを確認。
5. 日本語IMEの確定Enterだけでは追加されず、確定後のEnterで追加されることを確認。HTML風の項目名は一覧・履歴とも文字として表示されます。
